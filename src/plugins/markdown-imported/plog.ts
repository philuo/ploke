...
const _tableParser = (token: string, lang: string) => {
    ...
    if (/\s*[^\|]+\s*/.test(content[0])) {
        ...
        content[0]
            ...
            .split('|').slice(1, -1)
            .map(item => {
                result += `<th>${item.replace('&verticle_stroke;', '|')}</th>`;
            });
        result += '</tr></thead>';
    }
    result += '<tbody>';
    content.forEach(item => {
        ...
        .split('|').slice(1, -1)
        .map(el => {
            result += `<td><code>${el.replace('&verticle_stroke;', '|')}</code></td>`;
        });
        result += '</tr>'
    });
    result += '</tbody></table>';
 ...
 */
export const tokenify = (content: string, mode: boolean): PlogToken[] => {
    if (!content) return [];
    ...
    ((options: PlogOptions = {}) => {
        ...
        return {
            render(code: string, mode: boolean = false): string {
                const html = tokenify(code, mode).reduce(
                    (html: string, token: PlogToken) => {
                        switch (token.tag) {
                            case TOKEN_TAG.CODE: {
                                return (html += `<pre class="lang-${token.lang}"><code>${
                                    opts.codeblockParser(
                                        token.value,
                            ...
                            case TOKEN_TAG.SUBTITLE: {
                                return (html += opts.subtitleParser(
                                    token.value
                                ));
                            }
